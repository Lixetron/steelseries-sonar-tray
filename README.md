...
Sonar Tray is a **daily-driver layer on top of Sonar**, not a replacement for GG. Routing apps to channels, mic setup, spatial audio, and driver management still live in SteelSeries GG.
...
Settings persist immediately to disk.
...
You can edit the file while the app is running; reopen settings or restart to ensure all services pick up changes.
...
- [ ] **Custom tray icon** — currently uses the generic application icon
...
**In scope:** fast mixer access, media key redirection, overlay, visualizer, future Discord workaround.
...
4. Describe user-visible behavior in the PR.
